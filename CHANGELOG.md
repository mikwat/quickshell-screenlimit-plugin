...
Screen Limit forked from [Screen
Time](https://github.com/ax1g/quickshell-screentime-plugin) 1.6.2 and restarted
its version numbering at 1.0.0. Entries from 1.6.2 down are Screen Time's
history, kept for reference — they describe that plugin, so their IPC target
(`agx.screen-time`) and history path are the ones it used.
